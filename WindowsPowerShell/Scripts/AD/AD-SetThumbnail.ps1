			...
			If(Test-Path $ImageFile)
			{
				...
				If(!$Overwrite)
				{
					If(Test-Path $SaveLocation)
					{
						...
						{
							Write-Verbose "Image '$ImageFile' exists in destination location - skiped"
							Continue
				...
				$NewImage.Dispose()
				Write-Verbose "Image '$ImageFile' was resized from $($OldWidth)x$($OldHeight) to $($NewWidth)x$($NewHeight) and saved in '$SaveLocation'"
...
$photo = [byte[]](Get-Content $fd.FileName -Encoding byte)

if ($photo.Length -ge 100kb)
   {Set-ImageSize $fd.FileName -WidthPx 350 -HeightPx 350 -Destination C:\Windows\Temp -Overwrite
    Clear-Variable -Name photo
    $newfile = $fd.SafeFileName.ToString()
    $photo = [byte[]](Get-Content "C:\Windows\Temp\$newfile" -Encoding byte)
    Set-ADUser test -Replace @{ thumbnailPhoto = $photo}
	}
